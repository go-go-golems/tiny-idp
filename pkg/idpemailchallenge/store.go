...
type Store interface {
	...
	// ConsumeVerifiedEmailChallenge makes a previously verified evidence record
	// unavailable for any subsequent native effect. Callers must still provide
	// the original durable bindings; raw email addresses never authorize use.
	ConsumeVerifiedEmailChallenge(context.Context, string, VerificationBindings, time.Time) (VerifiedEmailEvidence, error)
