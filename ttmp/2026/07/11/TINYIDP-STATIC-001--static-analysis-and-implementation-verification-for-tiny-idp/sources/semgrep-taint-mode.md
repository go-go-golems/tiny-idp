...
[Metavariables](https://semgrep.dev/writing-rules/pattern-syntax#metavariables) used in `pattern-sources` are considered *different* from those used in `pattern-sinks`, even if they have the same name! See [Metavariables, rule message, and unification](https://semgrep.dev/writing-rules/data-flow/taint-mode/advanced#metavariables-rule-messages-and-unification) for further details.
...
| `by-side-effect` | { `false`, `true`, `only` } | `false` | See [Taint sources by side-effect](https://semgrep.dev/writing-rules/data-flow/taint-mode/advanced#taint-sources-by-side-effect). |
| `control` (Pro) 🧪 | { `false`, `true` } | `false` | See [Track control sources](https://semgrep.dev/writing-rules/data-flow/taint-mode/advanced#track-control-sources-). |
...
| `by-side-effect` | { `false`, `true`, `only` } | `false` | See [Taint sanitizers by side-effect](https://semgrep.dev/writing-rules/data-flow/taint-mode/advanced#taint-sanitizers-by-side-effect). |
...
| `at-exit` (Pro) 🧪 | { `false`, `true` } | `false` | See [Restrict taint to at-exit sinks](https://semgrep.dev/writing-rules/data-flow/taint-mode/advanced#restrict-taint-to-at-exit-sinks-). |
...
| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `by-side-effect` | { `false`, `true` } | `true` | See [Propagate without side-effect](https://semgrep.dev/writing-rules/data-flow/taint-mode/advanced#propagate-without-side-effect). |
...
[Semgrep](https://semgrep.dev/semgrep-pro-vs-oss) can perform interprocedural taint analysis, that is, track taint across multiple functions.
...
Using the CLI option `--pro`, Semgrep will perform interprocedural (across functions) as well as *inter* -file (across files) analysis. Inter-file analysis is only supported for [a subset of languages](https://semgrep.dev/supported-languages). For a rule to run interfile, it also needs to set `interfile: true`:
