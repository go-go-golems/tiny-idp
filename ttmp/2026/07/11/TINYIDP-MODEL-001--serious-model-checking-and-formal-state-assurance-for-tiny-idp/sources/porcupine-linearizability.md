...
Porcupine is a fast linearizability checker [used](#users) in both academia and industry for testing the correctness of distributed systems. It takes a sequential specification as executable Go code, along with a concurrent history, and it determines whether the history is linearizable with respect to the sequential specification. Porcupine also implements a visualizer for histories and linearization points.

[![Linearizability visualization demo](https://raw.githubusercontent.com/anishathalye/assets/master/porcupine/top-demo.png)](https://anishathalye.github.io/porcupine/ex-2.html)  
(click for interactive version)

Porcupine implements the algorithm described in [Faster linearizability checking via P-compositionality](https://arxiv.org/pdf/1504.00204.pdf), an optimization of the algorithm described in [Testing for Linearizability](http://www.cs.ox.ac.uk/people/gavin.lowe/LinearizabiltyTesting/paper.pdf).

Porcupine is faster and can handle more histories than [Knossos](https://github.com/jepsen-io/knossos) 's linearizability checker. Testing on the data in `test_data/jepsen/`, Porcupine is generally **1,000x** - **10,000x** faster and has a much smaller memory footprint. On histories where it can take advantage of P-compositionality, Porcupine can be millions of times faster.
...
See the [documentation](https://pkg.go.dev/github.com/anishathalye/porcupine) for how to write a [model](https://pkg.go.dev/github.com/anishathalye/porcupine#Model) and [specify histories](https://pkg.go.dev/github.com/anishathalye/porcupine#Operation). You can also check out some [example implementations](https://github.com/anishathalye/porcupine/blob/master/porcupine_test.go) of models from the tests.

Once you've written a model and have a history, you can use the [`CheckOperations`](https://pkg.go.dev/github.com/anishathalye/porcupine#CheckOperations) and [`CheckEvents`](https://pkg.go.dev/github.com/anishathalye/porcupine#CheckEvents) functions to determine if your history is linearizable. If you want to visualize a history, along with partial linearizations, you can use the [`Visualize`](https://pkg.go.dev/github.com/anishathalye/porcupine#Visualize) function.
...
[![Example 1](https://raw.githubusercontent.com/anishathalye/assets/master/porcupine/ex-1.png)](https://anishathalye.github.io/porcupine/ex-1.html)
...
[![Example 2](https://raw.githubusercontent.com/anishathalye/assets/master/porcupine/ex-2.png)](https://anishathalye.github.io/porcupine/ex-2.html)

See [`porcupine_test.go`](https://github.com/anishathalye/porcupine/blob/master/porcupine_test.go) for more examples on how to write models and histories.
...
[![Visualization demo](https://raw.githubusercontent.com/anishathalye/assets/master/porcupine/demo-annotations.png)](https://anishathalye.github.io/porcupine/demo-annotations.html)

You can see the full interactive version [here](https://anishathalye.github.io/porcupine/demo-annotations.html).
...
All that's needed to visualize histories is the [`CheckOperationsVerbose`](https://pkg.go.dev/github.com/anishathalye/porcupine#CheckOperationsVerbose) / [`CheckEventsVerbose`](https://pkg.go.dev/github.com/anishathalye/porcupine#CheckEventsVerbose) functions, which return extra information that's used by the visualization, and the [`Visualize`](https://pkg.go.dev/github.com/anishathalye/porcupine#Visualize) function, which produces the visualization. For the visualization to be good, it's useful to fill out the `DescribeOperation` and `DescribeState` fields of the model. See [`visualization_test.go`](https://github.com/anishathalye/porcupine/blob/master/visualization_test.go) for an end-to-end example of how to visualize a history using Porcupine.

You can also add custom annotations to visualizations, as shown in the example above. This can be helpful for attaching debugging information, for example, from servers or the test framework. You can do this using the [`AddAnnotations`](https://pkg.go.dev/github.com/anishathalye/porcupine#LinearizationInfo.AddAnnotations) method.

## Notes

- If Porcupine runs really slowly on your model/history, it may be inevitable, due to state space explosion. See [this issue](https://github.com/anishathalye/porcupine/issues/6) for a discussion of this challenge in the context of a particular model and history.
- When recording timestamps for operations, especially on ARM and other weakly-ordered architectures, you may need to use memory barriers or atomic operations to ensure accurate measurements and avoid spurious linearizability violations. See [this issue](https://github.com/anishathalye/porcupine/issues/40) for details.
...
Copyright (c) Anish Athalye. Released under the MIT License. See [LICENSE.md](https://github.com/anishathalye/porcupine/blob/master/LICENSE.md) for details.