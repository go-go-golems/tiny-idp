...
func (*module) TypeScriptModule() *spec.Module {
	return &spec.Module{
		...
		RawDTS: []string{
			...
			`export interface InvocationContext<I = unknown, C = Record<string, unknown>> { readonly input: Readonly<I>; readonly cap: C; readonly present: PresentationBuilders; readonly secret: SecretContext; readonly evidence: Readonly<Record<string, unknown>>; readonly commit: CommitBuilders; }`,
			...
			`export interface ProgramBuilder { capabilities(requirements: Record<string, CapabilityRequirement>): void; lambda<I = unknown, C = Record<string, unknown>>(id: string, spec: LambdaSpec<I, C>): LambdaHandle; workflow(id: string, spec: WorkflowSpec): void; provider(kind: ProviderKind, name: string, spec: ProviderSpec): void; }`,
