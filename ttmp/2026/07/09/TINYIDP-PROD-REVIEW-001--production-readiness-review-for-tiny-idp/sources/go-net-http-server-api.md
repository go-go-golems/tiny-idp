...
| Path | Synopsis |
| --- | --- |
| [cgi](https://pkg.go.dev/net/http/cgi@go1.26.5)  Package cgi implements CGI (Common Gateway Interface) as specified in RFC 3875. | Package cgi implements CGI (Common Gateway Interface) as specified in RFC 3875. |
| [cookiejar](https://pkg.go.dev/net/http/cookiejar@go1.26.5)  Package cookiejar implements an in-memory RFC 6265-compliant http.CookieJar. | Package cookiejar implements an in-memory RFC 6265-compliant http.CookieJar. |
| [fcgi](https://pkg.go.dev/net/http/fcgi@go1.26.5)  Package fcgi implements the FastCGI protocol. | Package fcgi implements the FastCGI protocol. |
| [httptest](https://pkg.go.dev/net/http/httptest@go1.26.5)  Package httptest provides utilities for HTTP testing. | Package httptest provides utilities for HTTP testing. |
| [httptrace](https://pkg.go.dev/net/http/httptrace@go1.26.5)  Package httptrace provides mechanisms to trace the events within HTTP client requests. | Package httptrace provides mechanisms to trace the events within HTTP client requests. |
| [httputil](https://pkg.go.dev/net/http/httputil@go1.26.5)  Package httputil provides HTTP utility functions, complementing the more common ones in the net/http package. | Package httputil provides HTTP utility functions, complementing the more common ones in the net/http package. |
| [internal](https://pkg.go.dev/net/http/internal@go1.26.5)  Package internal contains HTTP internals shared by net/http and net/http/httputil. | Package internal contains HTTP internals shared by net/http and net/http/httputil. |
| [ascii](https://pkg.go.dev/net/http/internal/ascii@go1.26.5) |  |
| [httpcommon](https://pkg.go.dev/net/http/internal/httpcommon@go1.26.5) |  |
| [testcert](https://pkg.go.dev/net/http/internal/testcert@go1.26.5)  Package testcert contains a test-only localhost certificate. | Package testcert contains a test-only localhost certificate. |
| [pprof](https://pkg.go.dev/net/http/pprof@go1.26.5)  Package pprof serves via its HTTP server runtime profiling data in the format expected by the pprof visualization tool. | Package pprof serves via its HTTP server runtime profiling data in the format expected by the pprof visualization tool. |