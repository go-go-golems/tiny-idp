...
LastUpdated: 2026-07-20T10:00:00-04:00
...
> continuations.
>
> **Implementation-reference notice (2026-07-20):** The executable v1 surface
> is the declaration generated by
> `internal/gojamodules/tinyidp/typescript.go`, exercised by
> `internal/gojamodules/tinyidp/typescript_test.go`, and demonstrated by
> `pkg/idpsignup/open_signup.js` and
> `pkg/idpsignup/email_verified_signup.js`. Sections 1--16 explain the
> architecture and deliberately use conceptual notation where useful. Section
> 17 is the normative copy-and-paste API reference; no older conceptual
> spelling (such as `ctx.present.signupForm` or a direct handler map) is an
> additional supported v1 API.
...
module.exports = A.program("open-signup", program => {
  const start = A.lambda("signup.start", {
    ...
    run: ctx => ctx.present.form({
      title: "Create an account", resume: "submitted",
  ...
  const submitted = A.lambda("signup.submitted", {
    ...
    run: ctx => ctx.commit.signup({
      login: ctx.input.email, displayName: ctx.input.displayName,
      password: ctx.secret.password, passwordConfirmation: ctx.secret.passwordConfirmation,
    }),
  });
...
This is the implemented v1 API. It is intentionally smaller than the
architectural vocabulary used in earlier explanatory sections: v1 does not
export `identity`, `subject`, `invite`, `error`, client registration, response
writing, or a generic effect constructor. A program declares lambdas, workflows
and providers; an invocation can return a bounded result, a native presentation,
an email challenge, or the closed native signup-commit request.

The maintained source of the declaration is
`internal/gojamodules/tinyidp/typescript.go`. The checked-in executable examples
are `pkg/idpsignup/open_signup.js` and
`pkg/idpsignup/email_verified_signup.js`; their compilation and execution are
covered by `pkg/idpsignup/executor_test.go`.

### 17.1 Module

```typescript
interface TinyIdpV1 {
  lambda<I = unknown, C = Record<string, unknown>>(
    id: string, spec: LambdaSpec<I, C>
  ): LambdaHandle;
  program(name: string, define: (program: ProgramBuilder) => void): unknown;
...
interface ProgramBuilder {
  ...
  workflow(id: string, spec: WorkflowSpec): void;
  provider(kind: ProviderKind, name: string, spec: ProviderSpec): void;
  test(id: string, spec: ProgramTestSpec): void;
}

interface WorkflowSpec {
  version: number;
  entry: string;
  handlers: Record<string, LambdaHandle>;
  edges?: Array<{
    from: string;
    outcome: "continue" | "present" | "challenge";
    to: string;
    input: string;
  }>;
}
...
```typescript
interface InvocationContext<I = unknown, C = Record<string, unknown>> {
  readonly input: I;
  readonly cap: C;
  readonly present: PresentationBuilders;
  readonly challenge: ChallengeBuilders;
  readonly secret: SecretContext;
  readonly evidence: Readonly<Record<string, unknown>>;
  readonly commit: CommitBuilders;
}

interface PresentationBuilders { form(spec: PresentationSpec): Outcome; }
interface ChallengeBuilders { emailCode(spec: EmailChallengeSpec): Outcome; }
interface CommitBuilders { signup(spec: SignupCommitSpec): Outcome; }
interface ResultBuilders {
  continue(handler: string): Outcome;
  present(spec: { handler: string; carry?: unknown; expiresInSeconds: number }): Outcome;
  challenge(spec: { handler: string; carry?: unknown; expiresInSeconds: number }): Outcome;
  commit(effects: unknown[]): Outcome;
  complete(value?: unknown): Outcome;
  deny(code: string): Outcome;
  skip(code?: string): Outcome;
  error(code: string): Outcome;
}
```

There is no `ctx.store`, `ctx.sql`, `ctx.fetch`, `ctx.fs`, `ctx.oauth`,
`ctx.tokens`, `ctx.signingKey`, `ctx.identity`, `ctx.subject`, `ctx.invite`, or
`ctx.effect`.

### 17.4 Continuation requirements

Every `ctx.present.form` or `ctx.challenge.emailCode` outcome must specify:
