...
module.exports = A.program("open-signup", program => {
  const start = A.lambda("signup.start", {
    ...
    run: ctx => ctx.present.form({ title: "Create an account", resume: "submitted", fields: [A.field.displayName(), A.field.email(), A.field.password(), A.field.passwordConfirmation()], actions: [A.action.submit(), A.action.deny()], carry: {}, expiresInSeconds: 300 }),
  });
