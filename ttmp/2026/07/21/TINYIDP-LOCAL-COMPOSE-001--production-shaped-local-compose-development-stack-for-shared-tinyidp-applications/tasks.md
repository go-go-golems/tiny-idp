...
- [x] Map the existing Compose demo, production manifests, and goja auth-host runtime contract. <!-- t:lera -->
- [x] Implement a strict local HTTPS proxy topology with both client registrations and per-client themes. <!-- t:e5uc -->
- [x] Document the local run, browser trust, reset, observability, and optional local goja build workflows. <!-- t:42sv -->
- [x] Validate Compose configuration, build and start the full stack, then exercise both OIDC applications. <!-- t:zq12 -->
- [x] Publish the intern guide to reMarkable and complete ticket bookkeeping. <!-- t:1f86 -->
...
- [x] Cover signup validation paths: malformed email, duplicate email, short password, password mismatch, email code failures, and invite failures. <!-- t:m17w -->
...
- [x] Classify browser-test failures in the defect ledger and implement the provider or application UX fixes without weakening security checks. <!-- t:8vbu -->
