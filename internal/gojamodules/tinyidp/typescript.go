...
func (*module) TypeScriptModule() *spec.Module {
	return &spec.Module{
		...
		RawDTS: []string{
			...
			`export interface InvocationContext<I = unknown, C = Record<string, unknown>> { readonly input: Readonly<I>; readonly cap: C; }`,
			...
			`export interface TinyIdpV1 { lambda<I = unknown, C = Record<string, unknown>>(id: string, spec: LambdaSpec<I, C>): LambdaHandle; program(name: string, define: (program: ProgramBuilder) => void): unknown; readonly result: ResultBuilders; }`,
