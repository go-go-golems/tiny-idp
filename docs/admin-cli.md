...
Client command output redacts stored secret hashes. Generated secrets are returned once in command output. For operator-managed or container-mounted secrets, use `--secret-file /run/secrets/<name>` instead of placing a value in argv. `--secret`, `--secret-file`, and `--generate-secret` are mutually exclusive. Secret files must be regular, non-symlink files no larger than 4096 bytes; surrounding whitespace is removed and empty files fail closed.
